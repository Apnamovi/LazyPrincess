...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [   
	    InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='http://t.me/ADownloadmovie/31')
        ]   
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            return await query.answer('♥️ Love @Mindsboss ♥️')
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('Movies Request Group', url=f'http://t.me/Apnamovie4')
            ],[
            InlineKeyboardButton('Movie Updates', url=f"https://telegram.me/LusiFilms"),
            InlineKeyboardButton('BOT Updates', url='https://t.me/LusiFilms')
            ],[
        ...
        buttons = [
            [
            InlineKeyboardButton("Donate LusiFilms", callback_data="linkdonatelazydev"),
            ],
        ...
        if 2 < len(message.text) < 100:
            ...
            if not files:
                ...
                l = await message.reply_text(text=f"△ 𝙷𝚎𝚢 𝚜𝚘𝚗𝚊 `{message.from_user.first_name}` 😎,\n\nʏᴏᴜʀ ʀᴇQᴜᴇꜱᴛ ʜᴀꜱ ʙᴇᴇɴ ꜱᴇɴᴛ ᴛᴏ ᴏᴜʀ **ᴀᴅᴍɪɴ'ꜱ ᴅᴀꜱʜʙᴏᴀʀᴅ** !\nᴘʟᴇᴀꜱᴇ ᴋᴇᴇᴘ ꜱᴏᴍᴇ ᴘᴀᴛɪᴇɴᴄᴇ !\nᴛʜᴇʏ ᴡɪʟʟ ᴜᴘʟᴏᴀᴅ ɪᴛ ᴀꜱ ꜱᴏᴏɴ ᴀꜱ ᴘᴏꜱꜱɪʙʟᴇ.\n\n➟ 📝𝘾𝙤𝙣𝙩𝙚𝙣𝙩 𝙣𝙖𝙢𝙚 : `{search}`\n➟ 👮𝙍𝙚𝙦𝙪𝙚𝙨𝙩𝙚𝙙 𝘽𝙮 : `{message.from_user.first_name}`\n\n༺ @LusiFilms ༻\n\n🦋・‥☆𝘼𝘿𝙈𝙞𝙉 𝙨𝙪𝙥𝙥𝙤𝙧𝙩☆‥・🦋\n╰┈➤・☆ @MindsBoss\n╰┈➤・☆ @LusiFilms",
                                                                                                       reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Click To Check Spelling ✅", url=f'http://www.google.com/search?q={search.text.replace(" ", "%20")}%20Movie')],[InlineKeyboardButton("Click To Check Release Date 📅", f'http://www.google.com/search?q={search.text.replace(" ", "%20")}%20Movie%20Release%20Date')]]))
